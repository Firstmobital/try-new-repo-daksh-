...
export default function ManageVariants() {
  ...
  useEffect(()=>{ (async ()=>{
    ...
    const { data: f } = await supabase.from("fuel_type").select("id,label,code");
    const { data: t } = await supabase.from("transmission").select("id,label,code");
  ...
  async function load(){
    ...
    setRows((data||[]).map(v=>({
      ...v,
      fuel_label: v.fuel?.label, transmission_label: v.tx?.label
    })));
  }
  async function save(){
    ...
    const { error } = await supabase.from("model_variant").upsert(payload, { onConflict:"id" });
    if (error) return alert(error.message);
  ...
  async function edit(r){
    setForm({
      id:r.id, car_id:r.car_id, name:r.name, fuel_type_id:r.fuel_type_id,
      transmission_id:r.transmission_id, ex_showroom:r.ex_showroom, image_url:r.image_url||"",
      is_published:r.is_published, features:r.features || ""
    });
  }
  ...
  async function exportCSV(){
    const csv = toCSV(rows.map(r=>({
      id:r.id, car_id:r.car_id, name:r.name, fuel_type_id:r.fuel_type_id, transmission_id:r.transmission_id,
      ex_showroom:r.ex_showroom, image_url:r.image_url, is_published:r.is_published, features:r.features
    })));
  ...
  async function importCSV(e){
    ...
    const payload = list.map(r=>({
      id:r.id||undefined, car_id:r.car_id, name:r.name,
      fuel_type_id:r.fuel_type_id, transmission_id:r.transmission_id,
      ex_showroom:Number(r.ex_showroom||0), image_url:r.image_url||null,
    ...
    <div className="space-y-6">
      ...
      <div className="card p-4 grid md:grid-cols-4 gap-2">
        ...
        <div className="col-span-full">
          <button className="btn btn-primary" onClick={save}>{form.id ? "Update" : "Add"} Variant</button>
        </div>
      </div>
